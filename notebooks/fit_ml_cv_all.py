...
for partition in partitions:
    ...
    # Load data
    train_set = pandas.read_pickle(os.path.join(os.environ['HOME'], 'data/post_mortem/crossval_feature_vectors/{subject_id}_{fwhm}_{train_set_name}.pkl'.format(**locals())))
    test_set = pandas.read_pickle(os.path.join(os.environ['HOME'], 'data/post_mortem/crossval_feature_vectors/{subject_id}_{fwhm}_{test_set_name}.pkl'.format(**locals())))
    ...
    path_name = os.path.join(os.environ['HOME'], 'data', 'post_mortem', 'ml_clusters_cross_validated')
    if not os.path.exists(path_name):
        os.makedirs(path_name)
    pickle_fn = os.path.join(os.environ['HOME'], 'data', 'post_mortem', 'ml_clusters_cross_validated', '%s_%s_%s_%s_%s.pkl' %(subject_id, fwhm, stain, n_clusters, partition['train_name']))
...
results_fn = os.path.join(os.environ['HOME'], 'data', 'post_mortem', 'ml_clusters_cross_validated', '%s_%s_%s_%s_all.pandas' %(subject_id, fwhm, stain, n_clusters)) #_all.pandas'.format(**locals()))
