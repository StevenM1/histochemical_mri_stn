...
df = pandas.read_pickle(os.path.join(os.environ['HOME'], 'data', 'post_mortem', 'gradient_dataframes', '{subject_id}_{fwhm}.pkl'.format(**locals())))
n_samples = 100000
n = 10000
...
s = SimpleExgaussMixture(x, n_clusters)

s.fit_multiproc(n_tries=14, n_proc=14)
#s.fit()

pickle_fn = os.path.join(os.environ['HOME'], 'data', 'post_mortem', 'ml_clusters_gradient', '{subject_id}_{fwhm}_{stain}_{n_clusters}.pkl'.format(**locals()))
pkl.dump(s, open(pickle_fn, 'w'))
